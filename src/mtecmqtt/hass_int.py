...
#    [ "Set general mode",         "MTEC_load_battery_btn",    "load_battery_from_grid" ],
  ...
  def initialize( self, serial_no ):
    ...
    self.device_info = { 
      ...
      "name": "MTEC Energybutler Inverter 1", 
      "manufacturer": "MTEC", 
      "model": "Energybutler",
      "via_device": "MTECmqtt" 
    }  
  ...
  def _build_automation_array( self ):
    ...
    for item in self.buttons:
      data_item = { 
        ...
        "command_topic": "MTEC_INVERTER_1/" + self.serial_no + "/automations/command",
  ...
  def _append_sensor( self, item ):               
    data_item = { 
      ...
      "state_topic": "MTEC_INVERTER_1/" + self.serial_no + "/" + item["group"] + "/" + item["mqtt"],
      "device": self.device_info
    }
