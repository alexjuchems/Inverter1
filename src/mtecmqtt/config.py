...
def create_config_file():
  ...
  if cfg_path: # Usually something like ~/.config/mtecmqtt/config.yaml resp. 'C:\\Users\\xxxx\\AppData\\Roaming'
    cfg_fname = os.path.join(cfg_path, "mtecmqtt", "config1.yaml")  
  else:
    cfg_fname = os.path.join(os.path.expanduser("~"), ".config", "mtecmqtt", "config1.yaml")  # ~/.config/mtecmqtt/config.yaml
...
def init_config():
  ...
  cfg_path = os.environ.get('XDG_CONFIG_HOME') or os.environ.get('APPDATA')
  if cfg_path: # Usually something like ~/.config/mtecmqtt/config.yaml resp. 'C:\\Users\\xxxx\\AppData\\Roaming'
    conf_files.append(os.path.join(cfg_path, "mtecmqtt", "config1.yaml"))  
  else:
    conf_files.append(os.path.join(os.path.expanduser("~"), ".config", "mtecmqtt", "config1.yaml"))  # ~/.config/mtecmqtt/config.yaml
